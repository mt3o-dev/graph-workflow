...
| 16 | `kartka-live-teacher-insights` | Host view aggregating weak-question data across a set's full live-quiz history (anonymized per-student — "Student N" labels, never a real display name), so a host can re-teach the actual gap. No new role: reuses the existing set-ownership model exactly, per an explicit design decision (`getOwnedSet`, same as everywhere else) rather than adding a teacher/roster concept. | **approved** — designed around slice 15's concurrent-write race class from the start (verified sound on independent re-check, including 8 repeated concurrency-test runs); one should-fix (anonymized-index tiebreak wasn't deterministic under the normal multi-player-same-round case) fixed with an id-based secondary sort + a genuine two-player determinism test. 319/319 tests, build ok. Ready to archive. |
| 17 | `kartka-live-homework-mode` | Asynchronous homework variant: host assigns a set with a deadline, students complete it independently at their own pace, leaderboard settles at/after the deadline. Built (and reviewed) by Opus, deliberately WITHOUT the WebSocket sidecar — plain Astro SSR/htmx against new persisted state, reusing only the pure scoring/domain functions from slices 11-13, not `RoomState`/`LiveSessionPort` (see `docs/ADR-homework-mode.md`). | **approved, clean** — first slice in this family with zero review-found bugs in its own logic (concurrency/timezone lessons from slices 15/16 applied from the start). Surfaced and fixed an unrelated pre-existing production bug in slice 16's insights page. 343/343 tests, build ok. **Closes out the full 17-slice roadmap.** |
